...
class ShaderFunctions
{
	...
	public function implement(funk:FunkinLua):Void
	{
		...
		funk.addLocalCallback("setSpriteShader", function(obj:String, shader:String):Bool
		{
			...
			var leObj:FlxSprite = FunkinLua.utils.getObjectDirectly(split[0]);

			if (split.length > 1) {
				leObj = FunkinLua.utils.getVarInArray(FunkinLua.utils.getPropertyLoop(split), split[split.length - 1]);
			}
	...
	public function getShader(obj:String):FlxRuntimeShader
	{
		...
		if (split.length > 1) {
			target = FunkinLua.utils.getVarInArray(FunkinLua.utils.getPropertyLoop(split), split[split.length - 1]);
		}
		else target = FunkinLua.utils.getObjectDirectly(split[0]);
