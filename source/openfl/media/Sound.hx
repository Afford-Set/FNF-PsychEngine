...
class Sound extends EventDispatcher
{
	...
	**/
	public static function fromFile(path:String):Sound
	{
		#if lime
		#if MP3_ALLOWED
		if (StringTools.endsWith(path, '.mp3'))
		{
			#if cpp
			var bytes:ByteArray = ByteArray.fromFile(path);
	...
	**/
	public static function loadFromFile(path:String):Future<Sound>
	{
		...
		{
			#if cpp
			return ByteArray.loadFromFile(path).then(function(bytes:ByteArray):Future<Sound>
			{
				var decodedBytes:ByteArray = MiniMP3.encodeWav(MiniMP3.decodeMP3(bytes));
