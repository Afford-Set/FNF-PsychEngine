...
class ModsMenuState extends MusicBeatState
{
	...
	override function create():Void
	{
		...
		var list:ModsList = Paths.parseModList();
		...
		while (i < modsList.length)
		{
			...
			{
				var path:String = 'ui/unknownMod';
				if (Paths.fileExists('images/unknownMod.png', IMAGE)) path = 'unknownMod';

				newMod.icon.loadGraphic(Paths.getImage(path));
			}
...
class ModMetaData
{
	...
	public function new(folder:String)
	{
		...
		var pack:Dynamic = Paths.getModPack(folder); // Try loading json
		...
		if (pack != null)
		{
			...
			if (pack.color != null) {
				color = FlxColor.fromRGB(pack.color[0] != null ? pack.color[0] : 170, pack.color[1] != null ? pack.color[1] : 0, pack.color[2] != null ? pack.color[2] : 255);
			}

			restart = pack.restart;
		}
	}
}