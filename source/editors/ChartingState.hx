...
class ChartingState extends MusicBeatUIState
{
	...
	function addAssets():Void
	{
		...
		var reloadNotesButton:FlxButton = new FlxButton(200, noteSplashesInputTextP2.y, 'Change Notes', function():Void
		{
	...
	private function addNote(strum:Null<Float> = null, data:Null<Int> = null, type:Null<Int> = null):Void
	{
		...
		if (FlxG.keys.pressed.CONTROL && noteData > -1) {
			_song.notes[curSec].sectionNotes.push([noteStrum, (noteData + Note.pointers.length) % Math.floor(Note.pointers.length * 2), noteSus, noteTypeIntMap.get(daType)]);
		}
	...
	private function saveEvents():Void
	{
		...
		if ((data != null) && (data.length > 0))
		{
			...
			#if MODS_ALLOWED
			_file.save(data.trim(), #if sys CoolUtil.convPathShit(Paths.modFolders('data/' + _song.songID + '/' + #end 'events.json' #if sys )) #end);
			#else
			_file.save(data.trim(), #if sys CoolUtil.convPathShit(Paths.getJson('data/' + song.songID + '/' + #end 'events.json' #if sys )) #end);
			#end
		}
	}
