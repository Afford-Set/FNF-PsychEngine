...
class WeekEditorState extends MusicBeatUIState
{
	...
	override function beatHit():Void
	{
		...
		{
			if (curBeat % char.danceEveryNumBeats == 0 && char.animation.curAnim != null && !char.confirmed) {
				char.dance();
			}
		}
	}
	...
	public static function saveWeek(weekFile:WeekFile):Void
	{
		...
		if (data.length > 0)
		{
			...
			#if MODS_ALLOWED
			_file.save(data.trim(), #if sys CoolUtil.convPathShit(Paths.modFolders('weeks/' + #end weekFileName + '.json' #if sys )) #end);
			#else
			_file.save(data.trim(), #if sys CoolUtil.convPathShit(Paths.getJson('weeks/' + #end weekFileName + '.json' #if sys )) #end);
			#end
		}
	}
