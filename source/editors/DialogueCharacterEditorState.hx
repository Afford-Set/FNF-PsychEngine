...
class DialogueCharacterEditorState extends MusicBeatUIState
{
	...
	function saveCharacter():Void
	{
		...
		if (data.length > 0)
		{
			...
			#if MODS_ALLOWED
			_file.save(data.trim(), #if sys CoolUtil.convPathShit(Paths.modFolders('portraits/' + #end characterName + '.json' #if sys )) #end);
			#else
			_file.save(data.trim(), #if sys CoolUtil.convPathShit(Paths.getJson('portraits/' + #end characterName + '.json' #if sys )) #end);
			#end
		}
	}
